...
async function validate_reset_password(req: any, res: any, next: Function) {
  ...
  return res.render("_partials/reset-password.ejs", {
    fields: fields,
...
router.post(
  ...
  async (req: any, res) => {
    ...
    await user?.change_password(new_hashed_password);
    res.send("Success!");
  }
);
...
router.post("/forgot-password", async (req: any, res) => {
  ...
  if (!user) {
    return res.render("_partials/forgot-password.ejs", {
      fields: {
        username: {
...
router.post(
  ...
  async (req, res) => {
    ...
    await user?.change_password(hashed_password);
